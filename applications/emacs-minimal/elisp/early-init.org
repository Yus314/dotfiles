...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; mac-ct does not discover hidden system fonts automatically.  Keep
  ;; existing font choices and use the system font only as a last fallback.
  ;; Set the default fontset before daemon clients create graphical frames.
  (when (and (eq system-type 'darwin) (fboundp 'set-fontset-font))
    (set-fontset-font t nil (font-spec :family ".AppleSystemUIFont") nil 'append))
  ;)
#+end_src
