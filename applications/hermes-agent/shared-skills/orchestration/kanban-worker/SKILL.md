...
## Shared integration board privacy

Profiles are not a hard privacy boundary for a shared Kanban board. Task bodies,
comments, paths, events, and completion metadata are durable cross-profile data.
On the integration board, publish only redacted work orders and compact
owner-attested results. Never paste ledger transactions/balances, raw health
streams, meal records, credentials, or sensitive attachment paths. If raw data
is required, work on a separate domain-owned board and return a sanitized
handoff. Tenant labels do not provide hard isolation.
