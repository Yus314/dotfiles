...
今回、改善案5種類を実装し、開発用6試合と新しい未使用12試合で比較しました。選んだ v8 候補は、未使用試合の平均資金が店舗順固定で +0.30%、通常ルールで −1.34%となり、採用を見送りました。**既定の実装・配布用 agent は v7 のままです。**

- [改善実験の結果・棄却理由](IMPROVEMENT_REPORT.ja.md)
- [v8 候補の実装](baselines/v8-candidate/policy.py) / [候補アーカイブ（未採用）](baselines/v8-candidate/submission.tar.gz)
...
今回の v7/v8 比較を、配布済み重みだけで再実行する場合：

```bash
.venv/bin/python fetch_data.py v8-validation-manifest.json
.venv/bin/python compare_variants.py --exported --phase local-v8-recorded \
  --manifest v8-validation-manifest.json --variants v8-comparison-variants.json --stride 1
.venv/bin/python compare_variants.py --exported --phase local-v8-endogenous \
  --manifest v8-validation-manifest.json --variants v8-comparison-variants.json \
  --town endogenous --skip-teacher
```

結果は `artifacts/improvement/<phase>/` に出ます。比較は各版の保存済み実装を読み込みます。開発用の各候補を再評価する設定は `reports/improvement/variants-*.json` にあります。`report_improvement.py` は保存済み結果から報告と図を生成します（図には Matplotlib が必要）。
