            ...
            for x, t in enumerate(row):
                    ...
                    if not animal and self.day <= 27 and self.hour <= 21:
                        for crop in CROPS:
                            ...
                            if (
                                ...
                                and self.count[crop] >= self.targets[crop]
                            ):
                                continue
                ...
                if crop:
                    ...
                    if (
                        ...
                        # Match the training label; crop identity remains in tile features.
                        tasks.append(
                            ("DIG", "" if OPTIONS["canonical_dig"] else crop, x, y, 1)
                        )
                elif ani:
    ...
    while pending:
        ...
        for idx in pending:
            ...
            for j in range(lo, hi):
                ...
                if (
                    op == "PLANT"
                    and c.targets is not None
                    and c.count[item] + plant_jobs[item] >= c.targets[item]
                ):
                    continue
                score = scores[j]
                ...
                if (
                    ...
                    if op != "DROP":
                        continue
                    score += 20
                if worker_best is None or score > worker_best[0]:
                    second_score = (
                        worker_best[0] if worker_best is not None else -math.inf
                    )
                    worker_best = (score, idx, j, key)
                elif score > second_score:
                    second_score = score
            if worker_best is None:
                continue
            # Prioritize workers whose next-best available task is much worse.
            priority = (
                (worker_best[0] - second_score, worker_best[0])
                if OPTIONS["assignment"] == "regret"
                else (worker_best[0],)
            )
            if best_priority is None or priority > best_priority:
                best_priority, best = priority, worker_best
        if best is None:
            for idx in pending:
