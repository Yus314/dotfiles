...
#+begin_src emacs-lisp :tangle yes
  ...
  ")))
    ...
    (let* ((current-time (current-time))
      ...
      ;; org-archive-location を設定
      (setq org-archive-location
            (format "%s::** やったこと" archive-file))
    ...
    (org-capture-templates
     . '(
         ...
         ("1" "Level 1" entry (file taskfile)
          "* LEVEL1 %^{title}[/]\n:PROPERTIES:\n:CATEGORY: %\\1\n:END:\n%?")
         ("w" "Weekly Report" entry (file+headline weekly-dir "今週のこと")
          "* 今週のこと
  ,** やったこと
  ,** 時間計測
  ,#+BEGIN: clocktable :scope agenda-with-archives :maxlevel 10 :lang \"ja\" :block lastweek :wstart 6 :level 4
...
#+begin_src emacs-lisp :tangle yes
  ...
  (let* ((current-time (current-time))
           ...
           (archive-dir (expand-file-name (format "~/dropbox/archive/%s/" year)))
           (archive-file (format "%s%s-%s.org" archive-dir start-day end-day)))
    (setq org-journal-dir  archive-dir)
    (setq org-journal-file-format  archive-file)
