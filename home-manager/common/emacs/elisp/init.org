...
#+begin_src emacs-lisp :tangle yes
    	       ...
    	       "文:\n%s\n
以下はユーザーが作成したメモです。  
このメモは、以下の点を満たすように整理してください。  
1. 明確なタイトルを持つこと、
2. 単一の明確なアイデアが簡潔に記述されていること、
3. 関連するアイデアへのリンクが適切に含まれていること、

添削を行い、簡潔に改善点を提案してください。  
ユーザーの書き方のスタイルを維持してください。  

修正例を示す場合は、次のように * 修正例 として示してくださいし。  
また、複数のメモに分割する場合は、それぞれを別の 見出しで示してください。  
